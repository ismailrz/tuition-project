...
$statement=null;
try{
       $statement=$conn->prepare("select * from tutor where fullname like ? or tutor_gender=? or district=? or subject=? or institute=? order by date desc ");
	    ...
	    <div class="container"  style="max-width: 22%;min-height: 50% ;float: left;margin-right: 12px">
          <form class="form-group">
        	<table>
        		...
        		<tr>
        			<td>
        			<?php echo"<img src='$row[image]' width='140' height='150'/>"?>
        			</td>
        		</tr>
        	</table>
          </form>
...
/*end of search session */
	...
	$statement=null;
	try{
	$statement=$conn->prepare("select * from tutor order by date desc");
	$statement->execute();
	 $table_row=$statement->fetchAll();
}catch(PDOException $e){
  echo $e->geuMessage();
