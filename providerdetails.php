...
if(isset($_POST["SearchButton"])){
	...
	try
	{
	  $statement = $conn->prepare("select * from provider where fullname Like ? or phone_mobile=? or  contactaddress=? or district=? or thana_upazila=? or tuitionarea=? or institute=? or subject=? order by date desc");
          ...
          <div class="container-fluidn" style="max-width: 30%;min-height: 50% ;float:left;">
                <form class="form-group">
                	<table><br>
                       ...
                       <tr><td>
                       		<?php echo"<b > Posted Date : </b> ".$row['date'];?>
                       </td></tr>
	...
	try
	{
	  $statement = $conn->prepare("select * from provider order by date desc");
	  $statement->execute();
	  $provider_rows = $statement->fetchAll(); 
	}
	catch(PDOException $e)
          ...
          <div class="container-fluidn" style="max-width:30% ;min-height: 50% ;float: left;">
                <form class="form-group">
                	<table><br>
        				...
        				<?php echo"<b style=margin-top:12px;> Posted Date : </b> ".$row['date'];?>
